...
class KpnQuarterlyReportsTool:
    name = "kpn_quarterlyreports_tool"

    def __init__(
        self,
        persist_dir: str | None = None,
        collection_name: str = "kpnquarterlyreports",
    ) -> None:
        self.persist_dir = persist_dir or StorageConfig().chroma_persist_dir
        self.collection_name = collection_name

    def can_handle(self, payload: ToolInput) -> bool:
        return payload.user_intent in ("annual", "both")
    ...
    def _to_evidence(self, item: dict) -> EvidenceItem:
        ...
        return EvidenceItem(
            tool_name=self.name,
            source_type=str(md.get("source_type", "kpnquarterlyreports")),
